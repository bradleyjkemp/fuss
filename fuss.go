...
// Fuzz fills an arbitrary data structure with data based on the seed
func (f *Fusser) Fuzz(is ...interface{}) {
	for _, i := range is {
		v := reflect.ValueOf(i)
		if v.Kind() != reflect.Ptr && v.Kind() != reflect.Interface {
			panic("cannot fuzz non-pointer value")
		}
...
func (f *Fusser) fussValue(v reflect.Value) {
	...
	case reflect.Struct:
		for i := 0; i < v.NumField(); i++ {
			...
			if !field.CanSet() {
				continue
				// Could use unsafe hacks to make this field set-able:
